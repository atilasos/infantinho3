...
class ClassViewPermissionsTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Warm up memberships for assertions
        cls.student.refresh_from_db()
        cls.prof.refresh_from_db()
    ...
    def test_only_professor_of_turma_can_promote(self):
        ...
        url = reverse('classes:add_student', args=[self.turma.id])
        response = self.client.post(url, {'user_id': self.guest.id})
        # Check redirect happens back to the class detail page
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('classes:class_detail', args=[self.turma.id]))
    ...
    def test_professor_of_turma_can_promote(self):
        ...
        self.assertEqual(response.url, reverse('classes:class_detail', args=[self.turma.id]), "Should redirect to class detail.")
        ...
        # Ensure membership was created and guest promoted
        self.assertTrue(
            ClassMembership.objects.filter(
                class_instance=self.turma,
                user=guest_user,
                role=ClassMembership.Roles.STUDENT,
            ).exists()
        )

    def test_class_membership_created_for_students_and_teachers(self):
        """Adding users to a class should create explicit ClassMembership records."""
        self.assertTrue(
            ClassMembership.objects.filter(
                class_instance=self.turma,
                user=self.student,
        ...
        self.assertTrue(
            ClassMembership.objects.filter(
                class_instance=self.turma,
                user=self.prof,
                role=ClassMembership.Roles.TEACHER,
            ).exists()
        )
    ...
    def test_manage_classes_get_access(self):
        ...
        self.assertEqual(url, '/turmas/gerir-turmas/')
