...
class ChecklistDetailView(View):
    ...
    def post(self, request, template_id):
        ...
        _notify_teachers_student_update(request, status, item, mark, comment_text)
...
class ChecklistTurmaView(View):
    ...
    def post(self, request, class_id, template_id):
        ...
        _notify_student_teacher_update(request, status, item, mark, comment_text)
...
def _teacher_emails(student_class):
    """Return teachers' email addresses for the given class."""
    return [teacher.email for teacher in student_class.teachers.all() if teacher.email]


def _notify_teachers_student_update(request, status_record, item, mark, comment_text):
    """Notify class teachers when a student updates a checklist item."""
    recipients = _teacher_emails(status_record.student_class)
    if not recipients:
        return

    student_name = status_record.student.get_full_name() or status_record.student.username
    template_name = status_record.template.name
    turma_url = request.build_absolute_uri(
        reverse('checklists:checklist_turma', args=[status_record.student_class.id, status_record.template.id])
    )
    subject = _('[Infantinho] {student} atualizou {template}').format(
        student=student_name,
        template=template_name,
    )
    message = _(
        'O aluno {student} marcou o objetivo "{item}" como "{status}".\n'
        'Comentário: {comment}\n\n'
        'Consultar progresso da turma: {url}'
    ).format(
        student=student_name,
        item=item.text,
        status=mark.get_mark_status_display(),
        comment=comment_text or _('(sem comentário)'),
        url=turma_url,
    )
    dispatch_notification(
        ...
        category='checklist_student_update',
        metadata={
            'class_id': status_record.student_class_id,
            'template_id': status_record.template_id,
            'student_id': status_record.student_id,
            'item_id': item.id,
...
def _notify_student_teacher_update(request, status_record, item, mark, comment_text):
    """Notify the student when a teacher validates or rectifies a checklist item."""
    student_email = status_record.student.email
    if not student_email:
        return

    template = status_record.template
    student_name = status_record.student.get_full_name() or status_record.student.username
    teacher_name = request.user.get_full_name() or request.user.username
    detail_url = request.build_absolute_uri(
        reverse('checklists:checklist_detail', args=[template.id]) + f'?highlight={item.id}'
    )
    subject = _('[Infantinho] O professor atualizou {template}').format(template=template.name)
    message = _(
        'O professor {teacher} atualizou o objetivo "{item}" para "{status}".\n'
        'Comentário: {comment}\n\n'
        'Revê a tua checklist: {url}'
    ).format(
    ...
    dispatch_notification(
        ...
        category='checklist_teacher_update',
        metadata={
            'class_id': status_record.student_class_id,
