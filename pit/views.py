...
def _send_notification(recipients, subject, message):
    ...
    try:
        ...
        # In development/tests we prefer printing rather than failing silently
        print(f"PIT notification failed: {exc}")
...
def _notify_teachers_submission(request, plan):
    ...
    _send_notification(recipients, subject, message)
...
def _notify_student_teacher_eval(request, plan):
    ...
    _send_notification([plan.student.email], subject, message)
